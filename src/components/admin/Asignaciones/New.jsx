...
export function NuevaAsignacion() {
  ...
  // 1. Estado para almacenar el valor del select
  ...
  const handleOnChange = async (evento) => {
    const { name, value } = evento.target;
  ...
  const handleSubmit = async (e) => {
    ...
    // Validar
    const validationErrors = validateFields(formInputs, validationRules);
    ...
    setIsLoading(true);
    try {
      const resp = await ctrlAssgn.saveAssignment(formInputs);
      // console.log(resp);
      if (resp?.status === 200) {
