...
export function EditTopic() {
  ...
  const getCategory = async () => {
    try {
      setIsLoading(true);
      const resp = await controllerCategory.getCategory(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setIsLoading(true);
      const resp = await controllerCategory.updateCategory(id, formData);
      if (resp?.status === 200) {
  ...
  useEffect(() => {
    if (dataParams) {
      setFormData({
        name_category: dataParams.name_category || "",
        date_created_category: obtenerFechaActual(),
      });
    }
  }, [dataParams]);
